import { type NextRequest, NextResponse } from "next/server";
import { fetchDriveImage } from "@/lib/drive-image";
import { recordDriveUsage } from "@/lib/drive-monitor";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type OpenGraphImageRouteContext = {
  params: Promise<{ id: string }>;
};

export async function GET(_request: NextRequest, { params }: OpenGraphImageRouteContext) {
  const { id } = await params;

  try {
    const driveRes = await fetchDriveImage(id);

    if (!driveRes) {
      return NextResponse.json({ error: "Not configured" }, { status: 500 });
    }

    if (!driveRes.ok) {
      const text = await driveRes.text();
      console.error("Drive API OG image fetch failed:", driveRes.status, text);
      return NextResponse.json({ error: "Failed to fetch OG image" }, { status: 502 });
    }

    recordDriveUsage("files.get", { fileId: id, source: "opengraph-image" });

    const body = driveRes.body;
    if (!body) {
      return NextResponse.json({ error: "No image data" }, { status: 502 });
    }

    return new NextResponse(body, {
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Cache-Control": "public, max-age=86400, s-maxage=604800, stale-while-revalidate=86400",
        "Content-Disposition": "inline",
        "Content-Type": driveRes.headers.get("content-type") ?? "image/jpeg",
      },
    });
  } catch (err) {
    console.error("OG image proxy error:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to fetch OG image" },
      { status: 500 }
    );
  }
}
